...
const ShopifyProductCard = ({ product, t }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#e8ddd0] p-3 flex flex-col gap-2 hover:shadow-md transition-shadow relative h-[290px]">
...
export default function ShopifyProductsPage() {
  ...
  const handleFilterChange = (name, val) => {
    if (name === "collectionId") {
      setCurrentPageInfo(null); // Reset pagination on collection change
    }
  ...
  return (
    <div className="p-5 flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-bold text-[#3a2a1a]">{t.shopifyProducts}</h2>
              <p className="text-[11px] text-[#9a8a7a]">Manage and sync products directly from your Shopify store</p>
            </div>
          </div>
          <button 
            onClick={() => { setCurrentPageInfo(null); fetchData(); }}
            disabled={loading}
            className="bg-[#8B6914] text-white text-[11px] font-bold px-4 py-2 rounded-lg hover:bg-[#6a5010] transition-colors flex items-center gap-2 disabled:opacity-50"
          >
            <span className={loading ? "animate-spin" : ""}>🔄</span> Sync Shopify
          </button>
        </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        {/* Pagination */}
        <div className="flex items-center justify-between bg-white px-4 py-3 rounded-xl border border-[#e8ddd0]">
          <div className="flex-1 flex justify-between sm:hidden">
             {/* Mobile simplified buttons could go here */}
          </div>
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="relative inline-flex items-center px-4 py-2 border border-[#e8ddd0] text-sm font-medium rounded-md text-[#3a2a1a] bg-white hover:bg-[#fcfaf7] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
                {t.previous}
              </button>
              <button
